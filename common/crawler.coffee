  ...
  class Crawler
    ...
    start: (urls = [], callback) ->
      unless @working
        ...
        console.log "⇡", "grey", "CRAWLER/#{@constructor.name}", "started"
        @queue url, callback for url in urls
        @working = true
        @_checkQueue()
      else
        console.log "⇡", "red", "CRAWLER/#{@constructor.name}", " still working."
    ...
    finish: ->
      ...
      console.log "⇣", "grey", "CRAWLER/#{@constructor.name}", "finished in #{seconds}s"
      @working = false

    queue: (url, callback, log = true) ->
      if log
        console.log "⇢", "grey", "CRAWLER/#{@constructor.name}", url.replace("http://www.", "")
